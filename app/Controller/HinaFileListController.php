 ...
 * 雛ファイルリスト
 * 
 * 雛ファイルリスト画面では雛ファイルリスト一覧を検索閲覧、および編集ができます。
 * 
 * 
 * @date 2015-9-16	新規作成 
 * @author k-uehara
 *
 */
class HinaFileListController extends CrudBaseController {

	/// 名称コード
	public $name = 'HinaFileList';
	
	/// 使用しているモデル
	public $uses = array('HinaFileList','CrudBase');
	
	/// オリジナルヘルパーの登録
	public $helpers = array('CrudBase');

	/// デフォルトの並び替え対象フィールド
	public $defSortFeild='HinaFileList.sort_no';
	...
	public function index() {
        ...
        // CrudBase共通処理（前）
		$crudBaseData = $this->indexBefore('HinaFileList');//indexアクションの共通先処理(CrudBaseController)
		
		//一覧データを取得
		$data = $this->HinaFileList->findData2($crudBaseData);

		// CrudBase共通処理（後）
		$crudBaseData = $this->indexAfter($crudBaseData);//indexアクションの共通後処理
		
		$hina_file_listGroupList = array(1=>'ペルシャ',2=>'ボンベイ',3=>'三毛',4=>'シャム',5=>'雉トラ',6=>'スフィンクス');
		$hina_file_list_group_json = json_encode($hina_file_listGroupList,JSON_HEX_TAG | JSON_HEX_QUOT | JSON_HEX_AMP | JSON_HEX_APOS);
		
		// 雛ファイルIDを取得する
		$hina_file_id = $crudBaseData['kjs']['kj_hina_file_id'];

		$this->set($crudBaseData);
		$this->set(array(
			'title_for_layout'=>'雛ファイルリスト',
				'data'=> $data,
				'hina_file_id'=> $hina_file_id,
			'hina_file_listGroupList' => $hina_file_listGroupList,
			'hina_file_list_group_json' => $hina_file_list_group_json,
		));
	...
	public function detail() {
		...
		$res=$this->edit_before('HinaFileList');
		$ent=$res['ent'];
	

		$this->set(array(
				'title_for_layout'=>'雛ファイルリスト・詳細',
				'ent'=>$ent,
		));
	 ...
	 */
	public function reg(){
		$res=$this->reg_before('HinaFileList');
		$ent=$res['ent'];
		
		$regMsg="<p id='reg_msg'>更新しました。</p>";

		//オリジナルバリデーション■■■□□□■■■□□□■■■□□□
		//$xFlg=$this->validHinaFileList();
		$xFlg=true;
		if($xFlg==false){
		...
		//★DB保存
		$this->HinaFileList->begin();//トランザクション開始
		$ent=$this->HinaFileList->saveEntity($ent);//登録
		$this->HinaFileList->commit();//コミット

		$this->set(array(
				'title_for_layout'=>'雛ファイルリスト・登録完了',
				'ent'=>$ent,
				'regMsg'=>$regMsg,
		));
	...
	public function ajax_reg(){
		...
		if(!empty($_FILES["upload_file"])){
			...
			$ent['hina_file_list_fn'] = $upload_file;
		}
	...
	public function ajax_delete(){
	   ...
	   if(isset($_POST['eliminate_flg'])) $eliminate_flg = $_POST['eliminate_flg'];
		...
		$this->HinaFileList->begin();
		if($eliminate_flg == 0){
		    $ent = $this->HinaFileList->saveEntity($ent); // 更新
		}else{
	...
	*/
	public function auto_save(){
		...
		// データ保存
		$this->HinaFileList->begin();
		$this->HinaFileList->saveAll($data);
		$this->HinaFileList->commit();
	...
	public function csv_fu(){
		...
		$this->csv_fu_base($this->HinaFileList,array('id','hina_file_list_val','hina_file_list_name','hina_file_list_date','hina_file_list_group','hina_file_list_dt','note','sort_no'));
	...
	private function getDataForDownload(){
        ...
        //セッションから検索条件情報を取得
        $kjs=$this->Session->read('hina_file_list_kjs');
        
        // セッションからページネーション情報を取得
        $pages = $this->Session->read('hina_file_list_pages');
		...
		//DBからデータ取得
	   $data=$this->HinaFileList->findData($kjs,$page_no,$row_limit,$sort_field,$sort_desc);
		if(empty($data)){
			return array();
		}
	...
	private function initCrudBase(){
		...
		$this->kensakuJoken=array(
			...
			array('name'=>'kj_hina_fp','def'=>null),
		...
		$this->kjs_validate=array(
				...
				'kj_hina_fp'=> array(
						'maxLength'=>array(
								...
								'message' => '雛型ファイルパスは1024文字以内で入力してください',
								'allowEmpty' => true
						),
				),
		...
		$this->field_data = array('def'=>array(
			...
			'id'=>array(
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'HinaFileList.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			),

			'hina_file_id'=>array(
					'name'=>'雛ファイルID',
					'row_order'=>'HinaFileList.hina_file_id',
					'clm_show'=>1,
			),
			'hina_fp'=>array(
					'name'=>'雛型ファイルパス',
					'row_order'=>'HinaFileList.hina_fp',
					'clm_show'=>1,
			),

			'sort_no'=>array(
				'name'=>'順番',
				'row_order'=>'HinaFileList.sort_no',
				'clm_show'=>0,
			),
			'delete_flg'=>array(
					'name'=>'削除フラグ',
					'row_order'=>'HinaFileList.delete_flg',
					'clm_show'=>1,
			),
			'update_user'=>array(
					'name'=>'更新者',
					'row_order'=>'HinaFileList.update_user',
					'clm_show'=>0,
			),
			'ip_addr'=>array(
					'name'=>'更新IPアドレス',
					'row_order'=>'HinaFileList.ip_addr',
					'clm_show'=>0,
			),
			'created'=>array(
					'name'=>'生成日時',
					'row_order'=>'HinaFileList.created',
					'clm_show'=>0,
			),
			'modified'=>array(
					'name'=>'更新日時',
					'row_order'=>'HinaFileList.modified',
					'clm_show'=>0,
			),
		));
